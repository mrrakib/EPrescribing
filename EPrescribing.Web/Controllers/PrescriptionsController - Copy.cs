...
namespace EPrescribing.Web.Controllers
{
    public class PrescriptionsController : Controller
    {
        ...
        [HttpGet]
        [AppAuthorization]
        public async Task<ActionResult> AddEditPrescriptionHistory(VMExistingPatient model, int prescriptionId = 0, string tab = "")
        {
        ...
        [HttpPost]
        [AppAuthorization]
        public ActionResult AddEditPrescriptionHistory(Prescription model, string tab = "")
        {
            if (model.Status != "E" && model.Status != "C")
            {
                model.Status = "H";
            }
            var addOrUpdate = _prescriptionService.AddOrUpdatesHistory(model);
            if (!addOrUpdate)
            {
            ...
            _message.success(this, "Patient History Updated Successfully!");
        ...
        public async Task<JsonResult> AutoCompleteGeneric(string term)
        {
            ...
            var result = allGeneric.Where(m => m.GenericName.ToLower().StartsWith(term.ToLower()));
            return Json(result, JsonRequestBehavior.AllowGet);
        }
